...
\rfoot{\color{bluetext} tinytest 1.4.1}
...
\date{\today{} | Package version 1.4.1}
...
Showing 1 out of 2 results: 1 fails, 1 passes (17ms)
\end{Soutput}
\end{Schunk}
...
\begin{Soutput}
  result                                                      call diff
1   TRUE if (at_home()) { expect_false(getwd() == get_call_wd()) } <NA>
2   TRUE                 expect_equal(Sys.getenv("hihihaha"), "8") <NA>
3   TRUE                    expect_equal(getOption("hihihaha"), 8) <NA>
  short           file first last
1  <NA> test_call_wd.R     3    5
2  <NA>   test_env_A.R     6    6
3  <NA>   test_env_A.R    11   11
\end{Soutput}
\end{Schunk}
...
\end{Sinput}
\begin{Soutput}
test_se.R.....................    2 tests 1 fails 2 side-effects  3ms
...
Showing 3 out of 4 results: 1 fails, 1 passes, 2 side effects (3ms)
\end{Soutput}
\end{Schunk}
